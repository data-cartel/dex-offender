...
pub mod ownable_upgradeable {
    ...
    impl<M: ::ethers::providers::Middleware>
        From<::ethers::contract::Contract<M>> for OwnableUpgradeable<M>
    {
    ...
    impl ::ethers::contract::EthLogDecode for OwnableUpgradeableEvents {
        fn decode_log(
            ...
            if let Ok(decoded) = InitializedFilter::decode_log(log) {
                return Ok(OwnableUpgradeableEvents::InitializedFilter(
                    decoded,
                ));
            }
            if let Ok(decoded) = OwnershipTransferredFilter::decode_log(log) {
                return Ok(
                    OwnableUpgradeableEvents::OwnershipTransferredFilter(
                        decoded,
                    ),
                );
            }
